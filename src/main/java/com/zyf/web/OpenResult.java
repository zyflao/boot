...
public class OpenResult {
	...
	/**
	 * 生成json字符串 新版本Rest接口几遍失败，也需要吐出 data属性
	 * 
	 * @return
	 */
	public String buildJsonNew() {
		Map<String, Object> build = new HashMap<String, Object>();
		build.put("msg", msg);
		build.put("retcode", retcode);
		if (adds.size() == 0) {
			build.put("data", new JSONObject());
		} else {
			build.putAll(adds);
		}
		return JSONObject.toJSONString(build);
	...
	public static OpenResult serviceError(String errorNo, String errorInfo) {
		...
		msg = errorInfo;
		/*
		 * try { if (StringUtils.isNotBlank(errorInfo)) { if
		 * (errorInfo.startsWith("[")) { String[] strArray =
		 * errorInfo.split("]"); if (strArray.length > 2) { msg =
		 * strArray[1].trim().substring(1); } // msg =
		 * errorInfo.split("]")[0].trim().substring(1) //
		 * +"] ["+errorInfo.split("]")[1].trim().substring(1); } else { String[]
		 * strs = errorInfo.split("\\["); if (strs.length > 1) { msg =
		 * strs[0].trim(); Boolean flagChinese=
		 * patternChinese.matcher(msg).matches(); if(!flagChinese){ for(String
		 * str:strs){ if(str.contains("]")){ msg =
		 * str.substring(str.indexOf("]")+1).trim(); } } } } } } } catch
		 * (Exception e) { return new OpenResult(retcode, msg); }
		 */
		return new OpenResult(retcode, msg);
	}
	...
	public static void main(String[] args) {
		...
		int errorNo = 0;
		// String errorInfo1 =
		// "可取现金不足[fund_account = 880000001295,money_type = 0,occur_balance = 5000.00,fetch_cash = 564.52]";
